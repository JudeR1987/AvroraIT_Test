...
/*
	...
	ORDERS (таблица образцов)
	поля:
		ORDNO (номер образца, уникальное значение)
		USRNAME (кто зарегистрировал, не уникален)
		
	ORDTASK (таблица испытаний назначенных на образец)
	поля:
		TESTCODE (код испытания, не уникален)
		ORDNO (номер образца, не уникален)
		TESTNAME (название, не уникален)
	
	Написать SQL запрос, который выведет:
		1)	все образцы и испытания по ним (поля для вывода ORDNO,
			USRNAME,TESTCODE,TESTNAME);
		2)	все образцы, на которые назначено испытание с кодом (TESTCODE)
			равным 123 (поле для вывода ORDNO);
		3)	всех пользователей, которые зарегистрировали образцы,
			на которые назначено испытание с названием начинающимся
			с 'Измерения р' (поле для вывода USRNAME).
*/
